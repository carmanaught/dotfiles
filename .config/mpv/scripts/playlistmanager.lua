...
-- We don't need to jump to another file here or move the cursor, so let's just seek to the
--   start of the file (changed from original script)
    if cursor~=plen-1 then
      mp.commandv("no-osd", "seek", 0, "absolute")
    end
--    mp.commandv("playlist-next", "weak")
  end
...
function add_keybinds()
  ...
  mp.add_forced_key_binding('ESC', 'hideplaylist1', hideplaylist)
  mp.add_forced_key_binding('l', 'hideplaylist2', hideplaylist)
end
...
function remove_keybinds()
  if settings.dynamic_binds then
    ...
    mp.remove_key_binding('hideplaylist1')
    mp.remove_key_binding('hideplaylist2')
  end
end
...
function handlemessage(msg, value, value2)
  ...
  if msg == "hide" then hideplaylist() ; return end
end
...
mp.add_key_binding('l', 'showplaylist', showplaylist)
